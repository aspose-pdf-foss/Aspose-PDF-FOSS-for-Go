...
- Page rendering to raster images (P1 of a phased pure-Go rasterizer, umbrella `pdf-go-61r`). `(*Page).RenderImage(RenderOptions) (image.Image, error)` rasterizes a page at a chosen DPI (default 150) over its CropBox; `RenderPNG`/`RenderJPEG`/`RenderGIF` encode it; `(*Document).RenderImage(pageNum, …)` renders by number. Aspose-style `Resolution` + `PngDevice`/`JpegDevice`/`GifDevice` with `Process(page, w)`. The renderer is dependency-free (own anti-aliased rasterizer + stroker). P1 covers vector graphics (paths, fills, strokes, Gray/RGB/CMYK colour, CTM); text, images, shadings, and clipping render in later phases (operators not yet supported are skipped, so any page still produces an image).
