...
- Page rendering to raster images (phased pure-Go rasterizer, umbrella `pdf-go-61r`). `(*Page).RenderImage(RenderOptions) (image.Image, error)` rasterizes a page at a chosen DPI (default 150) over its CropBox; `RenderPNG`/`RenderJPEG`/`RenderGIF`/`RenderBMP` encode it; `(*Document).RenderImage(pageNum, …)` renders by number. Aspose-style `Resolution` + `PngDevice`/`JpegDevice`/`GifDevice`/`BmpDevice` with `Process(page, w)`. The renderer is dependency-free (own anti-aliased rasterizer + stroker + BMP encoder). **P1** covers vector graphics (paths, fills, strokes, Gray/RGB/CMYK colour, CTM); **P2** adds Image XObjects (with `/SMask` alpha) and Form XObject recursion; **P3** adds text for embedded TrueType fonts (own `glyf` outline decoder + text-object state machine); **P4** renders non-embedded Standard-14 fonts from bundled metric-compatible families (Arimo/Tinos/Cousine, SIL OFL, Latin-subset, ~716 KB) so layout is preserved and serif/mono/bold/italic are distinct, with a `FontRepository` (`AddFontFolder`/`AddFontFile`/`AddSystemFonts`) to use exact installed fonts instead. Shadings and clipping render in later phases (unsupported operators are skipped, so any page still produces an image).
