...
- PDF/A conformance writer (phase 2) — `(*Document).ConvertToPDFA(format)` adjusts the document toward a PDF/A "b" level in place and returns the remaining-violations report: removes encryption, JavaScript/Launch actions and (for PDF/A-1) file attachments; sets annotation print flags; adds an sRGB ICC **OutputIntent** (the profile is generated in pure Go — no bundled `.icc`); and writes a `pdfaid` XMP packet. Fonts already embedded via `LoadFont` make the result fully conformant; Standard-14 fonts and PDF/A-1 transparency remain the caller's responsibility (surfaced in the report). Mirrors the intent of Aspose.PDF for .NET's `Document.Convert(PdfFormat)`. (`pdf-go-s2n`)
