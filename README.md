...
[**docs/feature_showcase.pdf**](docs/feature_showcase.pdf) is a single 12-page PDF generated by [`_examples/feature_showcase`](_examples/feature_showcase/main.go). One narrative covers text (with an embedded, subsetted Unicode TTF), image embedding, styled AcroForm fields, the full annotation gallery, redactions (mark-mode + applied), tables (multi-page with repeating headers and spans), a landscape chart, and the vector primitives — plus cross-cutting features like bookmarks, named destinations, page labels, Info + XMP metadata, font subsetting, and AES-256 encryption (the file itself is encrypted; an empty user password lets any viewer open it, while the owner password gates modify operations).
