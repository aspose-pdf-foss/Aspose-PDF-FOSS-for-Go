...
[**docs/feature_showcase.pdf**](docs/feature_showcase.pdf) is a single 13-page PDF generated by [`_examples/feature_showcase`](_examples/feature_showcase/main.go). One narrative covers text (with an embedded, subsetted Unicode TTF), image embedding, styled AcroForm fields, the full annotation gallery, redactions (mark-mode + applied), tables (multi-page with repeating headers and spans), a landscape chart, the vector primitives, and form/annotation flattening — plus cross-cutting features like bookmarks, named destinations, page labels, Info + XMP metadata, font subsetting, and AES-256 encryption (the file itself is encrypted; an empty user password lets any viewer open it, while the owner password gates modify operations).
...
- **Render to image** — rasterize pages to PNG/JPEG/GIF/BMP with a dependency-free, pure-Go anti-aliased renderer (`Page.RenderImage`/`RenderPNG`/… + Aspose-style `PngDevice`/`Resolution`). Shipped in phases — vector, images, and text today (with `FontRepository`/`AddFontFolder` for custom fonts); shadings and clipping in upcoming releases. Default 150 DPI, renders the CropBox
