...
- **Table detection** — `TableAbsorber` recognizes ruled tables straight from the page's vector geometry (no rasterization): borders drawn as strokes, rectangle edges or thin fills; dashed and per-cell borders; rowspan/colspan recovered from the rule grid. `Visit(page)` → `TableList()` → rows → cells with text fragments. Mirrors Aspose.PDF for .NET's `TableAbsorber`
