...
[**docs/feature_showcase.pdf**](docs/feature_showcase.pdf) is a single 14-page PDF generated by [`_examples/feature_showcase`](_examples/feature_showcase/main.go). One narrative covers text (with an embedded, subsetted Unicode TTF), image embedding, styled AcroForm fields, the full annotation gallery, redactions (mark-mode + applied), tables (multi-page with repeating headers and spans), a landscape chart, the vector primitives, form/annotation flattening, and a meta "Page Rendering" page whose thumbnails are renders of this very document drawn by the built-in renderer — plus cross-cutting features like bookmarks, named destinations, page labels, Info + XMP metadata, font subsetting, and AES-256 encryption (the file itself is encrypted; an empty user password lets any viewer open it, while the owner password gates modify operations).
...
- **Render to image** — rasterize pages to PNG/JPEG/GIF/BMP and single- or multi-page TIFF with a dependency-free, pure-Go anti-aliased renderer (`Page.RenderImage`/`RenderPNG`/`RenderTIFF`/…, `Document.RenderTIFF` for a whole document, plus Aspose-style `PngDevice`/`TiffDevice`/`Resolution`). Draws vector graphics, images (incl. stencil image masks, **CCITTFaxDecode** Group 3/4 fax, and **JBIG2** bilevel scans), text (embedded TrueType, CFF Type1C/CID `/FontFile3`, and Type3; Standard-14 and non-embedded fonts via bundled metric-compatible substitutes, with a `FontRepository`/`AddFontFolder`/`AddSystemFonts` for exact and installed fonts including `.ttc` collections and `.otf`/CFF; non-embedded Type0 substitution), axial/radial shadings, tiling patterns, clipping, constant alpha, soft masks, blend modes, Optional-Content visibility, and annotation appearances (AcroForm widgets, stamps, highlights, …); honours text rendering modes and inline images. Default 150 DPI, renders the CropBox
...
> **Status:** the renderer draws **vector graphics** (paths, fills, strokes, Gray/RGB/CMYK plus Separation/DeviceN colour, axial/radial **shadings**, tiling patterns, **clipping**, constant alpha, **soft masks**, **blend modes**, Optional-Content visibility), **images** (Image XObjects with soft-mask alpha, stencil **image masks**, **CCITTFaxDecode** Group 3/4 fax, **JBIG2** bilevel scans — symbol-dictionary/text-region + generic/refinement regions on the arithmetic path, and inline images), and **text** (embedded **TrueType** and **CFF** — Type1C simple and CID-keyed `/FontFile3` — plus **Type3** fonts; non-embedded fonts via bundled metric-compatible substitutes and a `FontRepository` for exact/installed fonts including `.ttc` collections and `.otf`/CFF; non-embedded Type0 substitution and synthesized ZapfDingbats checkbox/radio marks), and **annotation appearances** (AcroForm field widgets, stamps, highlights, free text, …). Unsupported elements (mesh shadings, isolated/knockout transparency groups, non-embedded Symbol, mixed Group 3 2-D fax) are skipped, so a page always renders. Output: **PNG / JPEG / GIF / BMP / TIFF** (single- and multi-page); DPI defaults to 150; the rendered region is the CropBox.
