...
- **Reusable Form XObjects** — `Document.CreateForm(w, h)` returns an `XForm` you draw into once (via `Canvas()`, the full page drawing API), then place on any number of pages and positions with `Page.AddForm(form, rect)` — a single shared content stream behind many `Do` invocations. Ideal for page templates, shared headers/footers and watermarks; mirrors Aspose.PDF for .NET's `XForm`
