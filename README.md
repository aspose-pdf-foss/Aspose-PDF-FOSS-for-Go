...
- **Render to image** — rasterize pages to PNG/JPEG/GIF/BMP with a dependency-free, pure-Go anti-aliased renderer (`Page.RenderImage`/`RenderPNG`/… + Aspose-style `PngDevice`/`Resolution`). Shipped in phases — vector graphics and images today; text, shadings, and clipping in upcoming releases. Default 150 DPI, renders the CropBox
