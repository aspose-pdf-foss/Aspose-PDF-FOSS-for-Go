...
- **Render to image** — rasterize pages to PNG/JPEG/GIF/BMP and single- or multi-page TIFF with a dependency-free, pure-Go anti-aliased renderer (`Page.RenderImage`/`RenderPNG`/`RenderTIFF`/…, `Document.RenderTIFF` for a whole document, plus Aspose-style `PngDevice`/`TiffDevice`/`Resolution`). Draws vector graphics, images, text (embedded TrueType and OpenType-CFF `/FontFile3`, Standard-14 via bundled metric-compatible fonts, with a `FontRepository`/`AddFontFolder`/`AddSystemFonts` for exact and installed fonts including `.ttc` collections and `.otf`/CFF), axial/radial shadings, clipping, constant alpha, and annotation appearances (AcroForm widgets, stamps, highlights, …); honours text rendering modes and inline images. Default 150 DPI, renders the CropBox
