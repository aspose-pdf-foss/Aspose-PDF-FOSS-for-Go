...
- **Stamps** — `TextStamp`, `ImageStamp`, and `PageNumberStamp` overlay (or underlay) content via `(*Page/*Document).AddStamp`, with opacity, centre-pivot rotation, alignment, and behind-content mode; page-number stamps format `{0}`/`{1}` for headers/footers. Mirrors Aspose.PDF for .NET's `Aspose.Pdf.Stamp` family
...
- **Text search & replace** — `SearchText` (literal / case-insensitive / RE2 regex, optional `Rectangle` region) returns per-match bounding boxes; `ReplaceText` finds and replaces occurrences, redrawing the replacement in a metric-compatible Standard-14 face
...
signature stays valid and the file ends up with multiple signatures.
