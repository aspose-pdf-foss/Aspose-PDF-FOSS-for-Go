...
**`sign.go` / `pkcs7_sign.go` / `sign_verify.go` / `pkcs7_verify.go` / `appearance_sign.go`** — digital signatures (ISO 32000-1 §12.8; epic `pdf-go-bm9`). One PKCS#7-detached signature (`/SubFilter /adbe.pkcs7.detached`), SHA-256, RSA or ECDSA keys, over an unencrypted document; invisible by default or a visible appearance block. Mirrors the intent of Aspose.PDF for .NET's `PdfFileSignature` + `SignatureCustomAppearance`
- `(*Document).Sign(opts SignOptions) error` — configures a signature applied on the next Save/WriteTo (like `SetEncryption`). `SignOptions` — `Certificate *x509.Certificate`, `PrivateKey crypto.Signer` (RSA/ECDSA — no `.p12` file needed; caller loads the key however), `Chain []*x509.Certificate`, `Reason`/`Location`/`ContactInfo`/`Name`, `SigningTime`. Signing an encrypted document is rejected
...
- CMS is hand-rolled on `encoding/asn1` (stdlib has no PKCS#7): `buildPKCS7Detached` (SignedData with contentType/messageDigest/signingTime signed attributes; signature over the SET-OF-re-tagged attrs) and `verifyPKCS7Detached` (parse, recover signer cert by issuer+serial, check messageDigest == SHA-256(content), verify RSA PKCS#1v1.5 / ECDSA). Validated against **OpenSSL** (`cms -verify`) and **pyHanko** (`INTACT:TRUSTED,UNTOUCHED`, coverage `ENTIRE_FILE`) — invisible and visible alike. Out of scope: PAdES/CAdES, RFC 3161 timestamps, LTV, DocMDP certification, multiple/incremental signatures, image/logo in the appearance
