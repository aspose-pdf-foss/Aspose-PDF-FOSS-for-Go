...
**`sign.go` / `pkcs7_sign.go` / `sign_verify.go` / `pkcs7_verify.go`** — digital signatures (ISO 32000-1 §12.8; epic `pdf-go-bm9`). Phase 1: one invisible PKCS#7-detached signature (`/SubFilter /adbe.pkcs7.detached`), SHA-256, RSA or ECDSA keys, over an unencrypted document. Mirrors the intent of Aspose.PDF for .NET's `PdfFileSignature`
