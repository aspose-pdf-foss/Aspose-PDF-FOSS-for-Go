...
// CCITT T.4 modified-Huffman run-length code tables (white and black), built
// once into lookup maps keyed by (nbits<<16 | code) → run length. Terminating
// codes (0–63) end a run; makeup codes (≥64) accumulate. The extended makeup
// codes (1792–2560) are shared by both colours.

var whiteCodes = map[uint32]int{}
var blackCodes = map[uint32]int{}

// ccittCode is one {run length, code bits, number of bits} table entry.
...
func ccittBuild(m map[uint32]int, tbl []ccittCode) {
	for _, c := range tbl {
		m[uint32(c.bits)<<16|uint32(c.code)] = c.run
	}
}

func init() {
	ccittBuild(whiteCodes, whiteTerm)
	ccittBuild(whiteCodes, whiteMakeup)
	ccittBuild(whiteCodes, sharedMakeup)
	ccittBuild(blackCodes, blackTerm)
	ccittBuild(blackCodes, blackMakeup)
	ccittBuild(blackCodes, sharedMakeup)
}

var whiteTerm = []ccittCode{
...
var whiteMakeup = []ccittCode{
	...
	{1088, 0xD6, 9}, {1152, 0xD7, 9}, {1216, 0xD8, 9}, {1280, 0x91, 9},
	{1344, 0x92, 9}, {1408, 0x93, 9}, {1472, 0x94, 9}, {1536, 0x95, 9},
	{1600, 0x96, 9}, {1664, 0x18, 6}, {1728, 0x97, 9},
}

var blackTerm = []ccittCode{
